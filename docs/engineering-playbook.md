...
Read AGENTS.md, the relevant scope row, architecture.md, database-design.md and existing code. Identify the user-visible behavior, exact server effect, role, audit event, synchronization state, and failure response before changing UI. Check decisions.md. If a pending decision changes stored meaning, settle it before a migration, unless [the delivery plan](delivery-plan.md#requirements-first-recommendations-2026-09-26) records a user authorization to use recommended working defaults for that named slice. Work in a small vertical slice and keep unrelated cleanup separate. Source documents and agreement details remain local, outside Git.

For every financial feature, write a short operation contract first: input, value units, preconditions, affected accounts/stock buckets, shop-scoped idempotency identity, expected version, audit event, output, reconciliation lookup, and compensating action. Include ownership and physical custody when a financier or repair is involved. Use at least one realistic worked example with expected money minor units and milligrams. Product and backend reviewers approve that example before implementation. Where the delivery-plan authorization applies, Codex review of the labeled recommendation is the review gate for that slice, and the historical worksheet stays unsigned.

When that authorization applies, read the cited requirements first and label every gap in the contract as a source fact or a recommended working default. Do not describe a default as a shop-domain signature, and do not complete the blank approval rows in the worked examples. A recommendation does not relax exact integer units, balanced journals, server atomicity, idempotency, row-level security, owner-only access, or the ban on direct client writes to authoritative financial tables. Decisions outside the named slice stay unresolved. Deployment still requires a direct authorization in the session that would deploy.
...
Current package commands are listed in README.md. Run scoped formatting, `flutter analyze`, and `flutter test` for Flutter changes; `npm run lint`, `npm test`, and `npx tsc -b` for admin changes; and meaningful SQL, RLS, and atomicity gates for affected database behavior. The local `supabase/` command `npm run test:rls` uses a disposable database and rollback-only fixtures. Connected development SQL execution and its cleanup evidence are described in the validation record. Connectivity scripts do not replace database tests. Add financial command and migration replay checks as that code lands. Debug builds and runtime checks may run when relevant. UI changes still require Arabic RTL review in both themes at relevant widths. Flutter Android, Windows, and iOS release builds and React production `npm run build` run only when the user explicitly asks; they are never routine task gates. An iOS release build requires macOS and owner-controlled credentials. Network checks must use authorized environments and never print secrets. Preserve historical evidence and report environment limitations. Prepare store disclosures and account-deletion/support URLs before submission.

For documentation-only work, review links, consistency with source and current code, and privacy, and run `git diff --check`. Documentation-only changes do not require application tests or builds. Do not imply those gates passed.
...
Review each change against the scope contract, dependency direction, transaction effects, RLS, idempotency, Arabic copy, RTL themes, and test evidence. Grok may implement a scoped task when authorized, but it does not commit or push. Codex reviews the diff and reruns the relevant routine validation gates before any commit. Neither agent publishes or deploys without direct authorization.
