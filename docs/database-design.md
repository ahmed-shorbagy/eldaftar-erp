...
These migrations are not yet applied or RLS-tested against a disposable Supabase instance on this host. The SQL test script under `supabase/tests/` is reviewable but unexecuted. Do not treat this draft as a verified production boundary. Financial groups below remain proposed; the worked posting examples are still unsigned.
## Entity groups
