...
`private.is_active_shop_member`, `public.list_my_shop_accounts`, and `public.is_platform_admin` no longer read `email_confirmed_at`. They still require a non-anonymous, non-deleted `auth.users` row, and the existing membership, grant, entitlement, and platform-admin rules. If the JWT presents `session_id`, it must match a non-expired `auth.sessions` row for `auth.uid()`. A missing session claim remains valid for transaction-local SQL fixtures. User metadata does not grant a role. `create_shop_account`, `create_staff_invitation`, and `accept_staff_invitation` are no longer executable by `anon`, `authenticated`, or `service_role`. `set_staff_permission` and `revoke_staff_member` are unchanged. Invitation binding and password recovery remain undesigned.
...
The ordered migrations through `20260924141455_list_my_shop_accounts_status.sql` are the applied development identity draft: shops, memberships, independent staff grants, one entitlement per shop, invitations, identity audit, and a separate platform-admin check. Supabase Auth provides identity. An expired entitlement permits authorized reads and denies shop writes. A new shop remains pending activation until an owner-controlled entitlement is provisioned; account setup does not invent a trial term. Those eight migrations were applied to development project `xchapwvmvoefriqcxtvn`; the earlier rollback-only scripts passed there on PostgreSQL 17.6. That run still enforced `email_confirmed_at` and is historical.

`20260926075930_egypt_owner_password_registration.sql` is the ninth applied development migration. With it, contact-confirmation timestamps stop authorizing access, legacy shop setup and both invitation RPCs stop being API-callable, and new owner profiles go through the reservation RPCs above. Invitation rows already stored have no acceptance path until a later design. Financial groups below remain proposed; the worked posting examples are still unsigned. Clean local and staging replay remain unverified.
## Entity groups
...
`public.list_my_shop_accounts()` is the non-revoked membership lookup used after Auth. Its result is still shop id, name, role, `subscription_expires_at`, and `entitlement_status`. PostgreSQL computes `pending` for no entitlement or a future start, `active` for `starts_at <= now() < expires_at`, and `expired` after expiry. This avoids using a client clock to decide shop access. The forward registration migration keeps that signature and stops requiring `email_confirmed_at`. A presented JWT `session_id` must still be a live `auth.sessions` row. The Flutter adapter treats missing or unknown status as an invalid response and closes the selected shop view on access failure. Both status and registration migrations are applied to development; the revised rollback-only tests pass. Staging and production rollout remain separate acceptance steps.
