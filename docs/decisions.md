...
| D25 | **Revised and accepted 2026-09-26:** Egypt-only; email/password or Egyptian phone/password on one Supabase Auth account; no email verification, OTP, or SMS confirmation. Owner signup requires owner name, business name, email, phone, governorate, and password. | Changes Auth, registration, profiles and invitation identity binding | Milestone 1 identity revision | [ADR 0002](adr/0002-egypt-password-auth.md); documentation accepted, implementation pending; recovery and staff invitation binding need separate design |
