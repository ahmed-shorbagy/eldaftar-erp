...
| D10 | **Closed 2026-09-26:** there are no employee or partner roles. The single owner account sees and operates that shop's data. | The product owner cancelled multi-user shop access | Milestone 1 identity model | [ADR 0003](adr/0003-owner-only-shop-access.md); keep owner/shop RLS, revocation, entitlement, and platform-admin isolation |
...
| D25 | **Revised and accepted 2026-09-26:** Egypt-only; email/password or Egyptian phone/password on one Supabase Auth account; no email verification, OTP, or SMS confirmation. Owner signup requires owner name, business name, email, phone, governorate, and password. | Changes Auth, registration, and profiles | Milestone 1 identity revision | [ADR 0002](adr/0002-egypt-password-auth.md); owner slice implemented in development, with [evidence and acceptance limits](operations/milestone-1-validation.md); password recovery still needs a separate design. Staff invitation binding was cancelled by [ADR 0003](adr/0003-owner-only-shop-access.md) |
...
| D28 | **Revised 2026-09-26:** one subscription covers one shop and its single owner account. | Entitlement is attached to the shop. There is no second shop user and no per-user grant | subscription schema and redemption | redeem a code for one shop; a second shop needs its own entitlement and its own owner account. See [ADR 0003](adr/0003-owner-only-shop-access.md) |
