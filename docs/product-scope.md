...
| CRM | Multiple phone numbers, contact search/sort, notes, special/open requests, VIP marker, weight summaries per karat, contact links | Access is scoped to the owning shop; summaries derive from confirmed transactions; sensitive phone exposure is limited | LEDGER, PAGES |
...
Quick actions include cash method transfer, scrap deduction, scrap-to-stock conversion, return, expense, and daily note. Each action has its own protected owner command. A return links the original transaction where available and reverses exact effects before any replacement entries. Notes may have text and private images. Day close requires the signed-in owner, displays expected versus counted cash and gold, captures discrepancy reason, and seals the day with an auditable correction path. Closing after local midnight still closes the selected open business day.
