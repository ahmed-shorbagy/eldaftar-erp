...
This is the scope inventory for ElDafttar ERP. It is not a claim that any workflow already works. The three root Word files are abbreviated here as BASIC (core problems and goals), LEDGER (daily ledger detail and embedded UI concepts), and PAGES (page-by-page requirements). AGREEMENT means the technical scope in the root PDF, excluding its private and commercial details. If wording differs, preserve the original files for the product owner to decide; record the decision in decisions.md. The agreement links to an external annex that has not been retrieved here; D27 requires a private version comparison before scope sign-off. The PAGES document says new features extend an existing application, but this repository contains a starter only. Inventory the deployed legacy application before claiming parity.
