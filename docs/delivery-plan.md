...
4. Verify both login identifiers return the same user ID; wrong passwords, duplicate/invalid identifiers, missing signup fields, invalid governorates, partial registration failure, timeout/retry, session expiry, and membership loss. Prove no verification or OTP is required or sent. Re-run scoped formatting, analysis, tests, and rollback-only backend authorization tests; record exact evidence in the requirements matrix and validation record. Do not rewrite validation evidence already recorded. Android, Windows, and iOS release builds, and the React production build, are opt-in and run only when the user explicitly asks.
...
## Requirements-first recommendations 2026-09-26

On 2026-09-26 the user authorized the next financial slice to be derived from the repository documents and the three local Word files. Details those sources leave open are filled with explicit recommended working defaults, so that slice does not wait for a decision workshop. A default is an authorized recommendation for the named slice. It is not a shop-domain signature, it does not fill the blank approval rows in [the worked examples](discovery/financial-worked-examples.md), and it does not change the gates in this plan.

The historical G0 signatures are absent; do not claim they were supplied. The user-authorized, documented working defaults replace the workshop prerequisite for this named implementation slice. Invoice numbering, custody, and obligations outside the slice remain later decisions. Milestone 1 has not passed as a whole. Authentication and owner-only access are implemented on development, and the remaining acceptance gaps in [the validation record](operations/milestone-1-validation.md) stay open: live device and session demonstrations, clean local replay and CI, separate environments, the all-screens prototype or an explicit D12 deferral, interactive onboarding and Help resume, and iOS signing. Password recovery and subscription activation remain separate later slices. Release builds stay opt-in. Preparing this plan does not deploy or migrate anything; the implementation prompt explicitly scopes development database changes when submitted as a user request.

## Next bounded slice

The next implementation slice is the Milestone 2 financial foundation: an owner-confirmed opening balance and the live daily-ledger balances that follow from that confirmation. The ready-to-paste prompt is [next-slice-prompt.md](operations/next-slice-prompt.md). Preparing that prompt does not start the slice and does not make any financial action operational. Sale, purchase, expense, quick transfer, and manual close remain later Milestone 2 work. The slice after the opening foundation is the first atomic sale with multiple lines and mixed tenders.
...
The first bounded slice is [the opening-balance prompt](operations/next-slice-prompt.md): exact units, the minimum shop-owned catalog and scrap buckets, one atomic opening command, the initial Cairo business day, and the confirmed ledger summary and feed. The remainder of this milestone stays in later slices.
...
Before reporting a milestone complete, rerun the routine gates for affected packages. Current baseline from README.md: in app, scoped Dart formatting, flutter analyze, and flutter test; in admin, npm run lint, npm test, and npx tsc -b; for affected database behavior, meaningful SQL, RLS, and atomicity gates. Debug builds may run when a compile or runtime check is relevant. Arabic RTL visual review in both themes and at the relevant widths remains required for UI changes; runtime visual checks may run when relevant. In supabase, connection checks exist but are network checks, not schema/RLS tests. Flutter Android, Windows, and iOS release builds and admin npm run build are opt-in and run only when the user explicitly asks; they are never automatic routine task gates. Add database test/migration gates before the first financial release. An iOS release build requires a macOS signing environment and an owner account; do not claim it passed from Windows. Documentation-only changes require a link, consistency, and privacy review and git diff --check, not application tests or builds. Historical validation records stay intact.
