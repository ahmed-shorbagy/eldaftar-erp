...
begin
  ...
  if (select count(*) from public.list_my_shop_accounts() where shop_id = v_shop and subscription_expires_at is null) <> 1 then
...
values (current_setting('test.command_shop_id')::uuid, now() - interval '1 day', now() + interval '1 day');
